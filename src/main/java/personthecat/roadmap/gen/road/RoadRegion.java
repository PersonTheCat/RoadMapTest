...
public class RoadRegion implements Iterable<RoadNetwork> {
  ...
  public RoadRegion(final short x, final short y) {
    this(x, y, (byte) 0, new ArrayList<>());
  }

  protected RoadRegion(final short x, final short y, final List<RoadNetwork> roads) {
    this(x, y, (byte) 0, roads);
  }

  private RoadRegion(final short x, final short y, final byte quads, final List<RoadNetwork> data) {
    this.x = x;
    this.y = y;
    this.quads = quads;
    this.data = data;
  ...
  public boolean containsPoint(final int x, final int y) {
    ...
    final int aY = regionToAbs(this.y);
  ...
  public void setQuadGenerated(final short qX, final short qY) {
    this.setQuadFlag(getQuadFlag(qX, qY));
  }
  ...
  public boolean hasQuad(final short qX, final short qY) {
    return this.hasQuadFlag(getQuadFlag(qX, qY));
  }
  ...
  public void copyQuadInto(final RoadRegion r, final short qX, final short qY) {
  ...
  public static RoadRegion loadFromDisk(final RoadMap map, final long seed, final int x, final int y) {
  ...
  private static RoadRegion fromReader(final RoadMap map, final ByteReader br) throws IOException {
    ...
    for (int i = 0; i < len; i++) {
      ...
      final int nY = br.readInt32();
      final RoadNetwork n = map.getNetwork(nX, nY);
      if (n != null) {
        data.add(n);
      } else {
        System.err.printf("Error loading network: %s,%s (it must have been deleted)", nX, nY);
      }
    }
    return new RoadRegion(x, y, quads, data);
  }

  @Override
  public String toString() {
    return "Region[" + this.x + ',' + this.y + ']';
  }
  ...
  public static byte getQuadFlag(final short qX, final short qY) {
    if (qX % 2 == 0) { // offset, so even quads are higher in region
      return qY % 2 == 0 ? UL_QUAD : UR_QUAD;
    }
    return qY % 2 == 0 ? DL_QUAD : DR_QUAD;
  }
}
